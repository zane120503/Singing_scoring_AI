...
# -*- coding: utf-8 -*-
"""
Test Find Actual Voice Start - Tim vi tri giong hat thuc su
"""
...
def find_actual_voice_start():
    ...
    try:
        ...
        # Tinh baseline tu 15 giay dau (intro)
        baseline_length = min(15, len(rms))
        ...
        for i in range(min(120, len(rms))):
            ...
            if energy_change > 3.0:  # Thay doi lon gap 3 lan standard deviation
        ...
        # Tim thay doi dau tien sau 20 giay (bo qua intro)
        for change in energy_changes:
            if change['time'] > 20.0:
                logger.info(f"\nEnergy change dau tien sau 20s: {change['time']:.2f}s")
                logger.info(f"   RMS: {change['rms']:.4f}")
                logger.info(f"   Change: {change['change']:.2f}")
                return change['time']
        
        # Fallback: tim thay doi dau tien sau 15 giay
        for change in energy_changes:
            if change['time'] > 15.0:
                logger.info(f"\nFallback - Energy change dau tien sau 15s: {change['time']:.2f}s")
                logger.info(f"   RMS: {change['rms']:.4f}")
                logger.info(f"   Change: {change['change']:.2f}")
                return change['time']
...
def analyze_voice_characteristics():
    """Phan tich dac trung giong hat"""
    try:
        # Test file Waiting For You
        test_file = "C:\\Users\\admin\\Downloads\\Waiting For You.mp3"
        
        logger.info("Phan tich dac trung giong hat...")
        ...
        # Tinh baseline tu 15 giay dau
        baseline_length = min(15, mfccs.shape[1])
        ...
        for i in range(mfccs.shape[1]):
            ...
            if mfcc_change > 50.0:  # Thay doi lon trong MFCC
                mfcc_changes.append({
                    'time': start_time,
        ...
        # Tim thay doi dau tien sau 20 giay
        for change in mfcc_changes:
            if change['time'] > 20.0:
                logger.info(f"\nMFCC change dau tien sau 20s: {change['time']:.2f}s")
                return change['time']
...
if __name__ == "__main__":
    print("=== TEST FIND ACTUAL VOICE START ===")
    
    # Tim vi tri giong hat thuc su
    print("\n1. Tim vi tri giong hat thuc su...")
    result1 = find_actual_voice_start()
    
    # Phan tich dac trung giong hat
    print("\n2. Phan tich dac trung giong hat...")
    result2 = analyze_voice_characteristics()
